...
RoundPathAction::RoundPathAction():radius(0), angleBegin(0), angleEnd(0), m_bIsRotate(true)
{
}

RoundPathAction::~RoundPathAction()
{
}

void RoundPathAction::startWithTarget(CCNode *pTarget)
{
...
RoundPathAction* RoundPathAction::create(float duration, float r, float angle_begin, float angle_delta, bool is_rotate)
{
	RoundPathAction * pRoundPathAction = new RoundPathAction();
	if(pRoundPathAction && pRoundPathAction->Init(duration, r, angle_begin, angle_delta, is_rotate))
    {
...
//这个函数得到初始化数据
bool RoundPathAction::Init(float duration, float r, float angle_begin, float angle_delta, bool is_rotate)
{
...
void RoundPathAction::update(float time)
{
    ////CCLog("RoundPathAction::update(time = %f)", time);
   ...
   // //CCLog("RoundPathAction::update temp = (%f, %f) rot = %f", temp.x, temp.y, rotation);
	if(m_bIsRotate)
	{
