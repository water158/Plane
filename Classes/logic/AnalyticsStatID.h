...
#ifndef __FERRIS_GAME__AnalyticsStatID__
#define __FERRIS_GAME__AnalyticsStatID__
...
public:
    ...
    //升级主机用钻石
    static const char* getHeroUpgradeUseGem(int type,int level);
    //升级僚机用金币
    static const char* getWingUpgtadeUseCoin(int type,int level);
    //通过僚机类型和等级获取僚机ID字符串
    static const char* getWingUpgradeUseGem (int type,int level);
    //解锁主机
    ...
    //获取购买钻石ID 参数为购买钻石的量
    static const char* getGemID(int gemNum);
    //获取购买永久双倍金币id
